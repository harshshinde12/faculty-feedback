...
export default function ManageForms() {
  const [data, setData] = useState<{ depts: any[]; programs: any[]; subjects: any[] }>({ depts: [], programs: [], subjects: [] });
  ...
  useEffect(() => {
    if (selections.deptId) {
      fetch(`/api/admin/program?deptId=${selections.deptId}`)
        .then(res => res.json())
        .then(programs => setData(prev => ({ ...prev, programs: Array.isArray(programs) ? programs : [], subjects: [] })));
    }
  }, [selections.deptId]);
  ...
  useEffect(() => {
    if (selections.programId && selections.academicYear) {
      fetch(`/api/admin/subject?programId=${selections.programId}&academicYear=${selections.academicYear}`)
        .then(res => res.json())
        .then(subjects => setData(prev => ({ ...prev, subjects: Array.isArray(subjects) ? subjects : [] })));
    }
  ...
  const handleSaveForm = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert("Error: " + (err.error || "Failed to publish form"));
      }
    } catch (e) {
      alert("Network Error");
    } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <Link href="/admin/feedback-forms" className="text-rose-600 hover:underline mb-4 inline-block">&larr; Back to Feedback Forms</Link>
        ...
        <div className="mt-8 border-t pt-6">
          ...
          <div className="mt-8 flex flex-col md:flex-row gap-4">
            ...
            <button
              ...
              disabled={loading}
              className="flex-1 bg-rose-600 text-white font-bold py-3 rounded-xl hover:bg-rose-700 shadow-lg transition-all active:scale-[0.98] disabled:opacity-50"
            >
              {loading ? "Publishing..." : "Publish Targeted Form"}
            </button>
          </div>
        </div>
      </div>
